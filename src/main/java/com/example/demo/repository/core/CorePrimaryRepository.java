...
@Repository
public class CorePrimaryRepository {
	...
	@Autowired
	public CorePrimaryRepository(@Qualifier("primary-db") DataSource dataSource) {
	...
	public GridData searchPagingGridData(String pagingSql, String outputSql, SqlParams params, Map<String, String> withClauses) {
		...
		if(CoreUtils.isNotEmpty(outputSql)) {
			...
			statement.append(" SELECT * FROM ").append(PAGING_CLAUSE_NAME).append(" ORDER BY ").append(ROWNUM_COLUMN).append(" ");
		}
